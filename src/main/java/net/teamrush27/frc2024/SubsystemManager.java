...
public class SubsystemManager implements ILooper {
        ...
        for (Subsystem s : mAllSubsystems) {
            ...
            if (!areDevicesOK) {
                ...
                DriverStation.reportWarning(
                        "Rewriting configuration after detecting configuration drift for subsystem: " + s.toString(),
                        false);
            }
    ...
    private class EnabledLoop implements Loop {
        ...
        public void onLoop(double timestamp) {
            ...
            mAllSubsystems.forEach(s -> s.outputTelemetry(false));
        }
    ...
    private class DisabledLoop implements Loop {
        ...
        public void onLoop(double timestamp) {
            mAllSubsystems.forEach(Subsystem::readPeriodicInputs);
            mLoops.forEach(l -> l.onLoop(timestamp));
            mAllSubsystems.forEach(Subsystem::writeDisabledPeriodicOutputs);
            mAllSubsystems.forEach(s -> s.outputTelemetry(true));

            // periodically check that device configurations are OK
            if ((Timer.getFPGATimestamp() - mLastCheckTimeSeconds) > Constants.RECHECK_DEVICE_INTERVAL_SECONDS) {
